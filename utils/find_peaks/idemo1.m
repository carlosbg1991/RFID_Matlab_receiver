...
tic;
MeasuredPeaks=ipeak(demodata,PeakD);
iPeakTime=toc;
...
disp(' ')
disp('Comparing the actual peak parameters with the measured values,')
disp('you can see that the PEAK POSITIONS ARE MEASURED MOST ACCURATELY')
disp('and the heights, widths and areas less accurately. The')
disp('width and area measurements are based on the assumption that the')
disp('peaks are Gaussian in shape; for non-Gaussian peaks, the peak')
disp('positions will still be accurate, but the width and area will')
disp('be only approximate. ')
disp(' ')
...
figure(1)
clf
disp('Using peakfit.m:')
disp(' ')
disp('The peakfit function, and its interactive cousin ipf.m, use ALL')
disp('THE DATA POINTS in each peak, not just the top part of the peak')
disp('as does iPeak. ')
disp(' ')
disp('>> FitResults=peakfit(demodata,0,0,4,1,0,10,0,0,0)')
NumPeaks=max(MeasuredPeaks(:,1));

start=[MeasuredPeaks(1,2) MeasuredPeaks(1,4) MeasuredPeaks(2,2) MeasuredPeaks(2,4) MeasuredPeaks(3,2) MeasuredPeaks(3,4) MeasuredPeaks(4,2) MeasuredPeaks(4,4)];
tic;
FitResults=peakfit(demodata,0,0,NumPeaks,1,0,1,start,0,0);
T=toc;
disp(['Elapsed time is ' num2str(T) ' seconds'])
disp('         Peak #    Position      Height       Width        Area')
FitResults=sortrows(FitResults,2)
if size(ActualPeaks)==size(MeasuredPeaks),
    AveragePercentErrors=mean(abs(100.*(ActualPeaks-FitResults)./ActualPeaks))
end
disp(' ')
disp('Comparing the FitResults from the peakfit function to the  ')
disp('MeasuredPeaks values from iPeak, you can see that PEAKFIT IS ')
disp('USUALLY MORE ACCURATE. It does take more time to compute and is ')
disp('limited to a maximum of 6 peaks per fit. More complex signals ')
disp('have to be fit in sections. However, peakfit can work in cases')
disp('where the peaks are so overlapped that there are no maxima for ')
disp('iPeak to detect.')
disp('  ')
disp('In this run, the errors of the peakfit method are lower')
disp(['than iPeak by about a factor of ' num2str(round(mean(AverageiPeakErrors(2:5)./AveragePercentErrors(2:5)))) ', but the execution time' ])
disp(['is longer than iPeak by about a factor of ' num2str(0.1*round(10*T/iPeakTime)) '.' ])

disp(' ')
fprintf(2,'Press any key to continue.....or press Ctrl-C to abort.\n')
pause
% ---------------------------------------------------------------------
% Generate the next test signal
x=[1:increment:500];
% For each simulated peak, specify the amplitude, position, and width
% and the overall random noise level.
amp=[100 200 300 400];  % Amplitudes of the peaks  (Change if desired)
...
% Now call iPeak, with specified value of all input arguments.
tic;
MeasuredPeaks=ipeak(demodata,0,40.0882,7.4702e-007,9,13,249.5,325);
iPeakTime=toc;
% Compare MeasuredPeaks to ActualPeaks
disp('-----------------------------------------------------------------')
disp('Now, for a harder problem: four strongly overlapping peaks.')
disp(' Same peaks as in the example above, except that the peaks are')
disp([' much closer. Random noise in signal = ' num2str(Noise)])
disp(' ')
disp('         Peak #    Position      Height       Width        Area')
ActualPeaks
disp(' ')
...
for n=1:SizeResults(1),
    indexn=val2ind(ActualPeaks(:,2),MeasuredPeaks(n,2));
    error(n,:)=100.*abs(ActualPeaks(indexn,:)-MeasuredPeaks(n,:))./ActualPeaks(indexn,:);
    error(n,1)=indexn;
...
disp(' ')
disp('iPeak finds only the peaks that give actual maxima in the overall')
disp(' signal, and the peak parameters for the ones it does find are not')
disp(' very accurate.')
disp(' ')
fprintf(2,'Press any key to continue.....or press Ctrl-C to abort.\n')
pause
...
figure(1)
clf
disp('Using peakfit.m, and specifying that there are 4 Gaussian peaks:')
disp('>> FitResults=peakfit(demodata,0,0,4,1,0,10,0,0,0)')
disp('Calculating.........')
tic;
FitResults=peakfit(demodata,250,320,4,1,0,10,0,0,0);
Tpeakfit=toc;
FitResults=sortrows(FitResults,2)
if size(ActualPeaks)==size(FitResults),
    AveragePeakfitError=mean(abs(100.*(ActualPeaks-FitResults)./ActualPeaks))
end
disp(['Elapsed time is ' num2str(Tpeakfit) ' seconds'])
disp(' ')
disp('Peakfit attempts to lock in to and measure all four peaks.')
disp(' The peak parameter errors are generally lower than for iPeak, ')
disp(' especially for width and area, but the elapsed time is longer. ')
disp('  ')
disp('In this run, the errors of the peakfit method are lower')
disp(['than iPeak by about a factor of ' num2str(round(mean(AveragePercentErrors(2:5)./AveragePeakfitError(2:5)))) ', but the execution time' ])
disp(['is longer than iPeak by about a factor of ' num2str(round(Tpeakfit/iPeakTime)) '.' ])
disp('  ')
fprintf(2,'Press any key to continue.....or press Ctrl-C to abort.\n')
disp(' ')
pause
...
disp('The parameter errors are even better if we know that the peak')
disp('width of the four peaks should be equal (peaks shape 6):')
disp(' ')
disp('>> FitResults=peakfit(demodata,0,0,4,6,0,10,0,0,0) ')
disp('Calculating.........')
tic;
FitResults=peakfit(demodata,250,320,4,6,0,10,0,0,0)
T6=toc;
if size(ActualPeaks)==size(FitResults),
    AveragePeakfitError=mean(abs(100.*(ActualPeaks-FitResults)./ActualPeaks))
end
disp(['Elapsed time is ' num2str(T6) ' seconds'])
iPeakErrors=AveragePercentErrors(2:5);
PeakfitError=AveragePeakfitError(2:5);
ErrorRatio=mean(iPeakErrors./PeakfitError);
disp('  ')
disp('In this run, the errors of the peakfit method are lower')
disp(['than iPeak by about a factor of ' num2str(round(ErrorRatio)) ', but the execution time' ])
disp(['is longer than iPeak by only a factor of ' num2str(round(T6/iPeakTime)) '.' ])
disp(' ')
disp('For an even more difficult problem, change the Noise in')
disp('line 11 to Noise=10 and re-run.')
disp('  ')
